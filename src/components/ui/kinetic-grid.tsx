 ...
 * KineticGrid
 *
 * A full-bleed canvas background: a subtle dot texture with an invisible
 * grid that warps toward the pointer and ripples on every click.
 *
 * Adapted for embedding: the canvas is `absolute` (not fixed) so it stays
 * inside its positioning parent — it never floats over content that
 * scrolls below. With prefers-reduced-motion it renders a single static
...
export default function KineticGrid({
  ...
  const prefersReducedRef = useRef<boolean>(
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      // Reduced motion: redraw the single static frame on resize — setting
      // canvas.width/height clears the canvas.
    ...
    // Accessibility: render one static frame, skip the animation loop and
    // the pointer interactions entirely.
    if (prefersReducedRef.current) {
      return () => {
  ...
  // the wrapper leaves the viewport and resume on return.

  useEffect(() => {
    const root = rootRef.current;
    if (!root || prefersReducedRef.current) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // Resume only if the loop is not already running.
