...
function useResponsiveValue(baseValue: number, mobileValue: number) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const target = isMobile ? mobileValue : baseValue;
      // Keep the wheel inside the viewport on narrow screens instead of
      // jumping straight to the mobile radius at a hard breakpoint.
      const maxByWidth = width * (isMobile ? 0.42 : 0.5);
      setValue(Math.round(Math.min(target, maxByWidth)));
    };
